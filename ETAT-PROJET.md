...
- **"[lien à vérifier]" partout au lieu de vraies sources** — le prompt demandait au modèle de recopier l'URL entière depuis les résultats Tavily ; tâche peu fiable (surtout modèles gratuits), qui dégénérait systématiquement en repli prudent. Repéré par Sandrine sur le premier vrai run avec traduction. Corrigé : `tavily-client.ts` numérote chaque résultat (`[réf:N]`) et fournit une table `réf → URL` ; le modèle n'a plus qu'à recopier un petit numéro, jamais l'URL elle-même ; `resolveSourceRefs` fait la substitution finale par du code, fiable à 100 %. Vérifié en conditions réelles (run daily + weekly) : 0 référence non résolue, 0 repli "lien à vérifier", vraies URLs (arstechnica.com, techcrunch.com…) dans le digest publié. Tests unitaires ajoutés (`src/__tests__/tavily-client.test.ts`).
